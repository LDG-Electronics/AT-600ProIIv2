...
extern void log_header(uint8_t msgLevel, const char *file, int line);
...
#define LOG_DEBUG(LOG_MESSAGE_BLOCK)                                           \
    if (L_DEBUG <= LOG_LEVEL) {                                                \
        log_header(L_DEBUG, __FILE__, __LINE__);                               \
        LOG_MESSAGE_BLOCK                                                      \
    }
#define LOG_INFO(LOG_MESSAGE_BLOCK)                                            \
    if (L_INFO <= LOG_LEVEL) {                                                 \
        log_header(L_INFO, __FILE__, __LINE__);                                \
        LOG_MESSAGE_BLOCK                                                      \
    }
#define LOG_WARN(LOG_MESSAGE_BLOCK)                                            \
    if (L_WARN <= LOG_LEVEL) {                                                 \
        log_header(L_WARN, __FILE__, __LINE__);                                \
        LOG_MESSAGE_BLOCK                                                      \
    }
#define LOG_ERROR(LOG_MESSAGE_BLOCK)                                           \
    if (L_ERROR <= LOG_LEVEL) {                                                \
        log_header(L_ERROR, __FILE__, __LINE__);                               \
        LOG_MESSAGE_BLOCK                                                      \
    }
#define LOG_FATAL(LOG_MESSAGE_BLOCK)                                           \
    if (L_FATAL <= LOG_LEVEL) {                                                \
        log_header(L_FATAL, __FILE__, __LINE__);                               \
        LOG_MESSAGE_BLOCK                                                      \
    }
